...
def ordinal_to_year_month_day(ordinals):
    ...
    """
    ordinals = np.array(ordinals, np.int32)
    ...
    months = months + np.where(months < 10, 3, -9)
    year += np.where(months <= 2, 1, 0)
...
def year_month_day_to_ordinal(year, month, day):
    ...
    # Take the fictional date of 1 March 0000 as reference and consider 1 March
    # as start of the year. This simplifies computations.
    year -= np.where(month <= 2, 1, 0)
    month += np.where(month > 2, -3, 9)
...
def is_leap_year(years):
    ...
    Tensor of bool type.
    """
    years = np.array(years, np.int32)
    def divides_by(n):
        return np.equal(years % n, 0)
    return np.logical_and(
        divides_by(4), np.logical_or(~divides_by(100), divides_by(400)))
...
def days_in_leap_years_between(start_date, end_date):
    ...
    def days_in_leap_years_since_1jan0001(date):
        ...
        n_leap_days += np.where(is_leap_year(date.year()), days_in_cur_year, 0)
        return n_leap_days
...
def leap_days_between(start_date, end_date):
    ...
    def leap_days_since_year_0(date_tensor):
        ...
        return leap_years_since_0 - np.where(needs_adjustment, 1, 0)
