...
class TensorWrapper(metaclass=abc.ABCMeta):
  ...
  def concat(cls, tensor_wrappers, axis):
    ...
    return cls._apply_sequence_to_tensor_op(
        lambda ts: np.concatenate(ts, axis), tensor_wrappers)
  ...
  def stack(cls, tensor_wrappers, axis=0):
    ...
    return cls._apply_sequence_to_tensor_op(
        lambda ts: np.stack(ts, axis), tensor_wrappers)
  ...
  def where(cls, condition, tensor_wrapper_1, tensor_wrapper_2):
    ...
    return cls._apply_sequence_to_tensor_op(
        lambda ts: np.where(condition, ts[0], ts[1]), tensor_wrappers)
  ...
  def expand_dims(self, axis):
    """See tf.expand_dims."""
    return self._apply_op(lambda t: np.expand_dims(t, axis))

  def reshape(self, shape):
    """See tf.reshape."""
    return self._apply_op(lambda t: np.reshape(t, shape))

  def identity(self):
    """See tf.identity."""
    return self._apply_op(np.identity)

  def broadcast_to(self, shape):
    """See tf.broadcast_to."""
    return self._apply_op(lambda t: np.broadcast_to(t, shape))

  def transpose(self, perm=None):
    """See tf.transpose."""
    return self._apply_op(lambda t: np.transpose(t, perm))

  def squeeze(self, axis=None):
    """See tf.squeeze."""
    return self._apply_op(lambda t: np.squeeze(t, axis))

#   def boolean_mask(self, mask, axis=None):
#     """See tf.boolean_mask."""
#     return self._apply_op(lambda t: np.boolean_mask(t, mask, axis=axis))
