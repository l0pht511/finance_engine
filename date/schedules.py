...
from finance_engine.date import date_finance
...
def _gen_periodic_schedule(start_date,
  ...
  if backward:
    start_date = start_date.broadcast_to(np.shape(end_date.ordinal()))
    tenor = tenor.broadcast_to(np.shape(end_date.ordinal()))
  else:
    end_date = end_date.broadcast_to(np.shape(start_date.ordinal()))
    tenor = tenor.broadcast_to(np.shape(start_date.ordinal()))
  ...
  schedule_len_upper_bound = (
      np.ceil(np.max(
          days_between / (tenor.quantity() * min_days_in_period)))).astype(np.int32)

  # Add the periods.
  if backward:
    # Subtract tenor * n, where n = n_max, ..., 2, 1, 0.
    tenors_expanded = tenor * np.arange(schedule_len_upper_bound - 1, -1, -1,
                                        dtype=np.int32)
    schedules = end_date - tenors_expanded
    # Prepend start_date to ensure we always include it.
    schedules = date_finance.DateFinance.concat((start_date, schedules),
                                              axis=-1)
    in_bounds = schedules.ordinal() >= start_date.ordinal()

    # Pad with start_date.
    schedules = date_finance.DateFinance.where(in_bounds, schedules, start_date)

    # Find how much we overestimated max schedule length and trim the extras.
    not_start_date = np.not_equal(schedules.ordinal(),
                                        start_date.ordinal())
    max_schedule_len_error = (
        np.min(np.where(not_start_date)[..., -1]) - 1)
    schedules = schedules[..., max_schedule_len_error:]
  else:
    # Add tenor * n, where n = 0, 1, 2, ..., n_max.
    tenors_expanded = tenor * np.arange(schedule_len_upper_bound,
                                        dtype=np.int32)
    schedules = start_date + tenors_expanded
    # Append end_date to ensure we always include it.
    schedules = date_finance.DateFinance.concat((schedules, end_date), axis=-1)

    in_bounds = schedules.ordinal() <= end_date.ordinal()

    # Pad with end_date.
    schedules = date_finance.DateFinance.where(in_bounds, schedules, end_date)

    # Find the actual schedule length and trim the extras.
    not_end_date = np.not_equal(schedules.ordinal(), end_date.ordinal())
    # max_schedule_len = np.max(np.where(not_end_date)[..., -1]) + 2
    max_schedule_len = np.max(np.where(not_end_date)) + 2
  ...
  if end_of_month:
    ...
    schedules = date_finance.DateFinance.where(where_cond,
...
def _gen_business_days(start_date, end_date, holiday_calendar, backward=False):
  ...
  if backward:
    ...
    days = np.arange(max_len, dtype=np.int32)
